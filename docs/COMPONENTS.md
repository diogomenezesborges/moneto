...
- JSON → parseGeneric (any bank JSON export)
- XLSX → parseGeneric (any bank XLSX export)
- CSV → parseGeneric (any bank CSV export)
- PDF → AI parser (Gemini Vision)

**Generic Parser**:

- `lib/parsers.ts` - `parseGeneric()` handles all file formats without bank-specific logic
...
```typescript
// Capitalizes first letter of each word — works for any bank name
