...
- `generic`: Bank-agnostic parser (parseGeneric handles CSV/JSON/XLSX)
...
model Bank {
  id        String   @id @default(cuid())
  name      String   // User-entered bank name (e.g. 'My Bank', 'Credit Union')
...
**Generic Import**: Any bank's CSV, JSON, or XLSX exports are supported via `parseGeneric()`. Users specify their bank name as free text during import.
